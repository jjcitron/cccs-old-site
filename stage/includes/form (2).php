<div class="theForm">
	<form action="http://leadsubmit.consolidatedcredit.org/addlead.aspx" method="post" id="contactform" >
	<fieldset>
		<input type="hidden" name="SuccessURL" value="thanks.php?from=<?php echo $thanks;?>" />   
		<input type="hidden" name="apikey" value="adad11ddf0cf769fc147ec30e66dba83" />  
		<input type="hidden" name="loginid" value="2300" /> 
		<input type="hidden" name="partnerid" value="2300" />
		<input type="hidden" name="appid" value="8" />   		
		...
		<div class="phoneContainer">
			<input class="form-control"  id="phoneNumber" type="tel" pattern="^(?:(?:\+?1\s*(?:[.-]\s*)?)?(?:\(\s*([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9])\s*\)|([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9]))\s*(?:[.-]\s*)?)?([2-9]1[02-9]|[2-9][02-9]1|[2-9][02-9]{2})\s*(?:[.-]\s*)?([0-9]{4})(?:\s*(?:#|x\.?|ext\.?|extension)\s*(\d+))?$" name="phone_cell" value="" placeholder="Phone Number*" required/>
			<select  class="form-control" name="phonetype" required onchange="javascript:document.getElementById('phoneNumber').name = this.value;">
				<option value="phone_cell">Cell</option><option value="phone_home">Home</option><option value="phone_work">Work</option>
			</select>
		</div>
		...
		<div class="yel">
			...
			<p class="fcta">Call right now and speak to a debt specialist</br></br><a href="tel:8005551212">1-800-555-1212</a></p>
		</div>
	</fieldset>
	</form>               
	</div>
...
if (!Modernizr.input.required ||  $.browser.msie || $.browser.safari) {
